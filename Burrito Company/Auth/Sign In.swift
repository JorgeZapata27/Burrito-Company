...
class Sign_In: UIViewController, UITextFieldDelegate, ASAuthorizationControllerPresentationContextProviding, ASAuthorizationControllerDelegate, GIDSignInDelegate {
    ...
    func OnboardingBoolean() {
        if UserDefaults.standard.bool(forKey: "onboardingSeen") == false {
            let sb = UIStoryboard(name: "Main", bundle: nil)
            let vc: UIViewController = sb.instantiateViewController(withIdentifier: "OnboardingScreen1Nav") as! OnboardingScreen1Nav
            UIApplication.shared.keyWindow?.rootViewController = vc
        } else {
            print("Hello")
        }
    ...
    @IBAction func mainButtonPressed(_ sender: UIButton) {
        if self.email!.text! == "management" && self.password!.text! == "burrito2020" {
            UserDefaults.standard.set(true, forKey: "isManagement")
            let sb = UIStoryboard(name: "Main", bundle: nil)
            let vc: UIViewController = sb.instantiateViewController(withIdentifier: "Ordering") as! OrderManagementController
            UIApplication.shared.keyWindow?.rootViewController = vc
        } else {
