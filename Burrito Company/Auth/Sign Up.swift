...
class Sign_Up: UIViewController, UITextFieldDelegate, ASAuthorizationControllerPresentationContextProviding, ASAuthorizationControllerDelegate , GIDSignInDelegate {
    ...
    @IBAction func appleButtonPressed(_ sender: UIButton) {
        ...
        let alertController = UIAlertController(title: "Wir brauchen deine Name!", message: "Wie heißen Sie?", preferredStyle: .alert)
        let cancel = UIAlertAction(title: "Abbrechen", style: .cancel, handler: nil)
        let ok = UIAlertAction(title: "Weiter", style: .default) { (action: UIAlertAction) -> Void in
            print("OK")
